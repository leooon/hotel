...
const controller = {
	...
	init: {
		...
		ux: () => {
			...
			controller.init.setupFloorZoom();
		},
		updateTime: () => {
			const now = Date.now();
...
controller.init.setupFloorZoom = () => {
	...
	canvas.addEventListener('scroll', () => {
	  const floors = [...document.querySelectorAll('.andar:not(.fudido)')];
	  let intersectingFloor = null;
  
	  // Find FIRST floor that actually contains the red line
	  floors.some(floor => {
		const rect = floor.getBoundingClientRect();
		if (rect.top <= redLineY && rect.bottom >= redLineY) {
		  intersectingFloor = floor;
		  return true; // Stop searching after first match
		}
		return false;
	  });
  
	  // Only change state if new floor intersects
	  if (intersectingFloor && intersectingFloor !== activeFloor) {
		// Reset previous
		if (activeFloor) {
		  anime({
			targets: activeFloor,
		...
		activeFloor = intersectingFloor;
	  }
	  // Reset if no floor intersects
	  else if (!intersectingFloor && activeFloor) {
		anime({
		  targets: activeFloor,
		  scale: 1,
		  duration: 200,
